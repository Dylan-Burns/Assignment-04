...
bool LinkedBag<ItemType>::removeSecondNode340() {
	...
	if (headPtr != nullptr) {
		...
		std::shared_ptr<Node<ItemType>> secondNode = headPtr->getNext(); //reference to second Node
...
bool LinkedBag<ItemType>::addEnd340(const ItemType& newEntry) {
	...
	if (currPtr->getNext() == nullptr) {
		std::shared_ptr<Node<ItemType>> newNode(new Node<ItemType>());
...
int LinkedBag<ItemType>::getCurrentSize340Recursive() const {
	...
	std::shared_ptr<Node<ItemType>> currPtr = headPtr;
	return getCurrentSize340RecursiveHelper(currPtr);
}
...
int LinkedBag<ItemType>::getFrequencyOf340Recursive(const ItemType& anEntry) const {
	std::shared_ptr<Node<ItemType>> currPtr{ headPtr };
	return getFrequencyOf340RecursiveHelper(currPtr, anEntry);
}
...
ItemType LinkedBag<ItemType>::removeRandom340() {
	...
	std::shared_ptr<Node<ItemType>> temp = currPtr->getNext();
	item = temp->getItem();
	remove(item);
	return item;
}
